...
function pushCommonFlags(flags, options, keys, isTTY2, logLevelDefault) {
...
function flagsForBuildOptions(options, isTTY2) {
  ...
  pushCommonFlags(flags, options, keys, isTTY2, "info");
  ...
  if (mainFields)
    flags.push(`--resolve-extensions=${mainFields.join(",")}`);
  if (external)
...
function flagsForTransformOptions(options, isTTY2) {
  let flags = [];
  let keys = Object.create(null);
  pushCommonFlags(flags, options, keys, isTTY2, "silent");
  let sourcemap = getFlag(options, keys, "sourcemap", mustBeStringOrBoolean);
...
function createChannel(streamIn) {
  ...
  let handleIncomingPacket = (bytes) => {
    if (isFirstPacket) {
      ...
      if (binaryVersion !== "0.7.8") {
        throw new Error(`Cannot start service: Host version "${"0.7.8"}" does not match binary version ${JSON.stringify(binaryVersion)}`);
      }
      return;
    }
  ...
  return {
    ...
    service: {
      build(options, isTTY2, callback) {
        let [flags, write, stdin, resolveDir] = flagsForBuildOptions(options, isTTY2);
        try {
      ...
      transform(input, options, isTTY2, fs2, callback) {
        let flags = flagsForTransformOptions(options, isTTY2);
        input += "";
        let start = (inputPath) => {
          try {
            let request = {
...
let startService = (options) => {
  ...
  let child = child_process.spawn(command, args.concat(`--service=${"0.7.8"}`), {
    cwd: process.cwd(),
    windowsHide: true,
...
let runServiceSync = (callback) => {
  ...
  callback(service);
  let stdout = child_process.execFileSync(command, args.concat(`--service=${"0.7.8"}`), {
    cwd: process.cwd(),
    windowsHide: true,
    input: stdin,
